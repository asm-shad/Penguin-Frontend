// components/Home/ShopByBrands.tsx
import Link from "next/link";
import Image from "next/image";
import { GitCompareArrows, Headset, ShieldCheck, Truck } from "lucide-react";
import { Title } from "../ui/text";
import { fetchPopularBrands } from "@/services/product/brand.actions";
...
const ShopByBrands = async () => {
  ...
  const brandsResult = await fetchPopularBrands();
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2.5">
        {brands?.map((brand) => (
          <Link
            ...
            href={{
              pathname: "/shop",
              query: { brand: brand.slug }, // Changed from brand?.slug?.current to brand.slug
            }}
          ...
          >
            {brand.imageUrl && (
              <Image
                src={brand.imageUrl} // Changed from urlFor(brand?.image).url() to brand.imageUrl
                alt={brand.name} // Changed from "brandImage" to brand.name
                width={250}
                height={250}
