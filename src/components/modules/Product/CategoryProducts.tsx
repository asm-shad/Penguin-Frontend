...
import { fetchProductsByCategorySlug } from "@/services/product/product.actions";
...
const CategoryProducts = ({ categories, slug }: Props) => {
  ...
  const fetchProducts = async (categorySlug: string) => {
    setLoading(true);
    try {
      const result = await fetchProductsByCategorySlug(categorySlug, {
        isActive: true,
        sortBy: "name",
        sortOrder: "asc",
      });
  ...
  return (
    <div className="py-5 flex flex-col md:flex-row items-start gap-5">
      {/* Original Design - Simple category buttons */}
